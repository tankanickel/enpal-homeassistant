...
#        if measurement == "inverter" and field == "Power.House.Total":
        ...
        # Battery charge level in kWh
        if field == "Energy.Battery.Charge.Load":
            to_add.append(EnpalSensor(field, measurement, 'mdi:battery', 'Enpal Battery Charge Load', config['enpal_host_ip'], config['enpal_host_port'], config['enpal_token'], 'energy', 'Wh'))
        # Does not exist on my system (Solar Rel 3.35.1)
